...
class BeamHandler(base_handler.BaseHandler):
  ...
  def create_pipeline(self, overwrite: bool = False) -> None:
    ...
    if overwrite:
      # Check if pipeline exists when updating.
      self._check_pipeline_folder(pipeline_name)
    else:
      # Check if pipeline already exists.
      self._check_pipeline_folder(pipeline_name, required=False)
